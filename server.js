...
function normalizeStatus(raw) {
  ...
  if (s.includes('אלקטיבי'))                                 return 'elective';
...
function buildStats(filteredRows, allRows) {
  ...
  // --- status totals ---
  const statusCounts = { open: 0, closed: 0, emergency: 0, emergency_day: 0, elective: 0, unknown: 0 };
  ...
  for (const r of deduped) {
    ...
    if (!commandStats[r.command]) {
      commandStats[r.command] = { open: 0, closed: 0, emergency: 0, emergency_day: 0, elective: 0, unknown: 0, total: 0 };
    }
    commandStats[r.command][r.status]++;
    commandStats[r.command].total++;

    if (!perDate[r.reportDateStr]) {
      perDate[r.reportDateStr] = { open: 0, closed: 0, emergency: 0, emergency_day: 0, elective: 0, total: 0 };
    }
